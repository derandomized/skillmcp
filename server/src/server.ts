...
const uiHtml: string = readFileSync(join(here, "..", "ui", "browse.html"), "utf8")
...
function loadUiHtml(): string { return uiHtml; }

export function buildServer(): McpServer {
  const server = new McpServer(
    { name: "skillmcp", version: "0.1.0" },
    {
      instructions:
        "SkillMCP is a provider-agnostic skills marketplace. Use list_skills / search_skills to browse, " +
  ...
  server.registerTool(
    ...
    async ({ category }) => {
      ...
      const text =
        `${cat.marketplace.displayName} — ${plugins.length} plugin(s)` +
  ...
  server.registerTool(
    "get_skill",
    {
      title: "Get skill",
      description:
        "Return the full SKILL.md for a skill (by skill or plugin name). The caller may follow these " +
    ...
    async ({ name }) => {
      ...
      if (!p) return { content: [{ type: "text", text: `Unknown skill '${name}'.` }], isError: true };
      const parts = await Promise.all(
        p.skills.map(async (s) => `### ${s.path}\n\n${await readSkillMarkdown(s)}`)
      );
  ...
  server.registerTool(
    "install_instructions",
    {
      ...
      description:
        "Exact, copy-pasteable steps to install a SkillMCP plugin on a given surface " +
  ...
  server.registerResource(
    "skill",
    new ResourceTemplate("skillmcp://skills/{plugin}/{skill}", {
      list: async () => {
        const cat = await getCatalog();
        return {
          resources: cat.plugins.flatMap((p) =>
            p.skills.map((s) => ({
    ...
    { title: "SkillMCP skill", description: "Full SKILL.md of a marketplace skill", mimeType: "text/markdown" },
    async (uri, { plugin, skill }) => {
      const cat = await getCatalog();
  ...
  server.registerResource(
    "catalog",
    "skillmcp://catalog",
    { title: "SkillMCP catalog", description: "Machine-readable catalog (JSON)", mimeType: "application/json" },
    async (uri) => ({
